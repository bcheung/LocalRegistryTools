...
function register(package::Union{Module, AbstractString},
    ...
    if isnothing(pkg.name)
        # package does not have a Project.toml file
        pkg.name = last(splitpath(package))
        @info "Package Name" pkg.name
        _manifest = read_manifest(manifest_path)
        pkg_entry = get_pkg_entry(_manifest, pkg.name)
        pkg.uuid = UUID(pkg_entry["uuid"])
        if isnothing(VersionNumber(pkg_entry["version"]))
          pkg.version = v"1"
        else
          pkg.version = VersionNumber(pkg_entry["version"])
        end

        if !isnothing(pkg_entry["deps"])
          deps = Dict()
          for dep_name in pkg_entry["deps"]
            deps[dep_name] = UUID(get_pkg_entry(_manifest, dep_name)["uuid"])
          end
          pkg.deps = deps
        end
        @info "Package" pkg
        pkg.compat = Dict("julia"=>"1.0")
    end
    ...
    for (key, value) in pkg.compat
      @info "Compat Entry" key value
    end
...
function get_remote_repo(package_path, gitconfig)
    ...
    # pkg_path = "$(package_path)/.git"
    # @info "Local package path" pkg_path
    # return pkg_path
end
...
function read_manifest(path_or_stream)
  ...
  try
      if path_or_stream isa String
          ...
          raw_manifest = TOML.parsefile(path)
      else
          raw_manifest = TOML.parse(path_or_stream)
      end
  catch err
      path = path_or_stream isa String ? path_or_stream : ""
      if err isa TOML.ParserError
          error("Could not parse manifest $path: $(err.msg)")
      elseif all(x -> x isa TOML.ParserError, err)
          error("Could not parse manifest $path: $err")
      else
          rethrow()
      end
  end
...
function get_pkg_entry(raw_manifest::Dict, package_name::String)::Dict{String, Any}
  local pkg_entry
  try
    pkg_entry = raw_manifest[package_name][1]
    @info "Package Entry" pkg_entry
  catch err
    error("Could not find package: $package_name")
  end
  return pkg_entry
end
